 ...
 */
    ...
    public class DeclarationAnalyzer
    {
            ...
            for (int i = 0; i < _semi.Count; i++)
            {
                switch (_semi[i])
                {
                    ...
                    case "override":
                        ...
                        _currentcursor++;
                        break;
                    case "extern":case "explicit":
                    case "implicit":case "operator":
                    case "const":case "out":
                    case "ref":case "param":
                    case "in":case "params":
                    case "readonly":case "sealed":
                    case "volatile":case "unsafe":
                    case "this":case "dynamic":
                        _currentcursor++;
                        _prefix.Add(_semi[i]);
                        break;
                }
        ...
        bool erasePrefix()
        {
            if (_semi.Count < 2)
                return true;
            List<string> _prefix = ParsePrefix();
            List<string> newsemi = new List<string>();
            for (int i = 0; i < _semi.Count; i++)
            {
